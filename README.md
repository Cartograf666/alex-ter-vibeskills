...
[![Version](https://img.shields.io/badge/version-0.3.0--alpha.1-7c3aed)](VERSION)
[![Agent Skills](https://img.shields.io/badge/standard-Agent%20Skills-2563eb)](https://agentskills.io/specification)
[![License](https://img.shields.io/badge/license-MIT-16a34a)](LICENSE)

[Quick start](#quick-start) · [Which skill when](#which-skill-when) · [The six skills](#the-six-skills) · [Installation](#installation) · [Security](#security-model)
...
For the OpenAI Skills UI in ChatGPT, upload the same six `.skill` archives from [`packages/`](packages/) through **Profile → Skills → Create → Upload from your computer**. This is another delivery surface for the same OpenAI/Codex skill format, not a separate implementation of this toolkit.
...
flowchart LR
    ...
    I -->|ordinary host| J["Ready for human review"]
    I -->|capable trusted host| K["Attested accepted run"]
    I -->|repair| G
```
...
If you do not want to track the sequence yourself, run `$guide-development-pipeline` at any point. It reads the `.ai/` artifacts, reports which stage the project is in, and names the single next step. It is read-only: it routes, and never approves or implements.
...
The skills are intentionally separated by authority. Interviewing does not authorize implementation, implementation cannot silently rewrite its own contract, and routing cannot approve anything at all.

| Skill | Use it for | Produces |
|---|---|---|
| [`guide-development-pipeline`](skills/guide-development-pipeline/) | Find the current stage and the single next step | Position report and routing decision (read-only) |
...
- installs all six skills together;
...
- replaces only directories carrying its matching `.vibeskills-managed.json` ownership marker;
...
| Host capability | Maximum honest result |
|---|---|
| Ordinary chat or CLI host with deterministic gates | `READY_FOR_HUMAN_REVIEW` |
| Host with a fresh independent reviewer but no protected HMAC key boundary | `VERIFIED_NOT_ATTESTED` |
| Host with contract-required reviewer independence and trusted HMAC key custody outside model/tool context | Attested `ACCEPT` |

This repository ships the contracts, validators, and signing CLI, but not a hosted orchestrator, provider transport, credentials, reviewer accounts, or trusted key service. A runtime must supply those capabilities and record the actual provider/model identities.
